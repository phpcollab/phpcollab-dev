...
class tasks_model extends CI_Model {
	...
	function get_rows($flt, $num, $offset, $column) {
		$query = $this->db->query('SELECT IF(tsk.tsk_date_due IS NOT NULL AND tsk.tsk_date_due <= ? AND stu.stu_isclosed = ?, 1, 0) AS tsk_overdue, stu.stu_isclosed, prj.prj_name, trk.trk_name, mln.mln_name, mbr.mbr_name, mbr_assigned.mbr_name AS mbr_name_assigned, tsk.* FROM '.$this->db->dbprefix('tasks').' AS tsk LEFT JOIN '.$this->db->dbprefix('projects').' AS prj ON prj.prj_id = tsk.prj_id LEFT JOIN '.$this->db->dbprefix('trackers').' AS trk ON trk.trk_id = tsk.trk_id LEFT JOIN '.$this->db->dbprefix('milestones').' AS mln ON mln.mln_id = tsk.mln_id LEFT JOIN '.$this->db->dbprefix('members').' AS mbr ON mbr.mbr_id = tsk.tsk_owner LEFT JOIN '.$this->db->dbprefix('members').' AS mbr_assigned ON mbr_assigned.mbr_id = tsk.tsk_assigned LEFT JOIN '.$this->db->dbprefix('statuses').' AS stu ON stu.stu_id = tsk.tsk_status WHERE '.implode(' AND ', $flt).' GROUP BY tsk.tsk_id ORDER BY '.$this->session->userdata($column.'_col').' LIMIT '.$offset.', '.$num, array(date('Y-m-d'), 0));
		return $query->result();
	}
	function get_row($tsk_id) {
		$row = $this->db->query('SELECT IF(tsk.tsk_date_due IS NOT NULL AND tsk.tsk_date_due <= ? AND stu.stu_isclosed = ?, 1, 0) AS tsk_overdue, stu.stu_isclosed, prj.prj_name, trk.trk_name, mln.mln_name, mbr.mbr_name,  mbr_assigned.mbr_name AS mbr_name_assigned, tsk_parent.tsk_name AS tsk_name_parent, tsk.* FROM '.$this->db->dbprefix('tasks').' AS tsk LEFT JOIN '.$this->db->dbprefix('projects').' AS prj ON prj.prj_id = tsk.prj_id LEFT JOIN '.$this->db->dbprefix('trackers').' AS trk ON trk.trk_id = tsk.trk_id LEFT JOIN '.$this->db->dbprefix('milestones').' AS mln ON mln.mln_id = tsk.mln_id LEFT JOIN '.$this->db->dbprefix('members').' AS mbr ON mbr.mbr_id = tsk.tsk_owner LEFT JOIN '.$this->db->dbprefix('members').' AS mbr_assigned ON mbr_assigned.mbr_id = tsk.tsk_assigned LEFT JOIN '.$this->db->dbprefix('tasks').' AS tsk_parent ON tsk_parent.tsk_id = tsk.tsk_parent LEFT JOIN '.$this->db->dbprefix('statuses').' AS stu ON stu.stu_id = tsk.tsk_status WHERE tsk.tsk_id = ? GROUP BY tsk.tsk_id', array(date('Y-m-d'), 0, $tsk_id))->row();
		if($row) {
