...
class Auth_library_default extends CI_Driver {
	...
	function login($email, $password) {
		...
		$query = $this->CI->db->query('SELECT mbr.* FROM '.$this->CI->db->dbprefix('members').' AS mbr LEFT JOIN '.$this->CI->db->dbprefix('organizations').' AS org ON org.org_id = mbr.org_id WHERE mbr.mbr_email = ? AND mbr.mbr_authorized = ? AND org.org_authorized = ? GROUP BY mbr.mbr_id', array($email, 1, 1));
		if($query->num_rows() > 0) {
			$member = $query->row();
