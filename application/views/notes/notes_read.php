<article class="title">
	...
	<?php if($row->action_delete) { ?><li><a href="<?php echo $this->my_url; ?>notes/delete/<?php echo $row->nte_id; ?>"><i class="fa fa-trash-o"></i><?php echo $this->lang->line('delete'); ?></a></li><?php } ?>
...
<article id="<?php echo $this->router->class; ?>-read"<?php if($this->input->cookie($this->router->class.'-read') == 'collapse') { ?> style="display:none;"<?php } ?>>
	<div class="column half">
		...
		<?php if($row->nte_datecreated) { ?><?php echo $this->my_library->timezone_datetime($row->nte_datecreated); ?> (<span class="timeago" title="<?php echo $this->my_library->timezone_datetime($row->nte_datecreated); ?>"></span>)<?php } else { ?>-<?php } ?>
		</p>
	</div>
	<div class="column half">
