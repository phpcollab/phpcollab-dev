<article class="title">
	...
	<ul>
	<li><a href="<?php echo $this->my_url; ?>milestones/calendar/<?php echo $row->mln_id; ?>"><i class="fa fa-<?php echo $this->config->item('phpcollab/icons/calendar'); ?>"></i><?php echo $this->lang->line('calendar'); ?></a></li>
	...
	<?php if($row->action_delete) { ?><li><a href="<?php echo $this->my_url; ?>milestones/delete/<?php echo $row->mln_id; ?>"><i class="fa fa-trash-o"></i><?php echo $this->lang->line('delete'); ?></a></li><?php } ?>
...
<article id="<?php echo $this->router->class; ?>-read"<?php if($this->input->cookie($this->router->class.'-read') == 'collapse') { ?> style="display:none;"<?php } ?>>
	<div class="column third">
		...
		<?php if($row->mbr_name) { ?><?php if($row->mln_owner == $this->phpcollab_member->mbr_id) { ?><i class="fa fa-<?php echo $this->config->item('phpcollab/icons/owner'); ?>" title="<?php echo $this->lang->line('icon_owner'); ?>"></i><?php } ?><?php echo $row->mbr_name; ?><?php } else { ?>-<?php } ?>
		</p>
	</div>
	<div class="column third">
		<p>
		...
		<?php if($row->mln_description) { ?><?php echo nl2br($row->mln_description); ?><?php } else { ?>-<?php } ?>
		</p>
	</div>
	<div class="column third">
		<p>
		...
		<?php if($row->mln_date_due) { ?><?php if($row->mln_overdue == 1) { ?><i class="fa fa-<?php echo $this->config->item('phpcollab/icons/overdue'); ?>" title="<?php echo $this->lang->line('icon_overdue'); ?>"></i><strong><?php echo $row->mln_date_due; ?></strong><?php } else { ?><?php echo $row->mln_date_due; ?><?php } ?><?php } else { ?>-<?php } ?>
		</p>
		<p>
