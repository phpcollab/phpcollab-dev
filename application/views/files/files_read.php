...
<article class="title">
	...
	<?php if($row->action_delete) { ?><li><a href="<?php echo $this->my_url; ?>files/delete/<?php echo $row->fle_id; ?>"><i class="fa fa-trash-o"></i><?php echo $this->lang->line('delete'); ?></a></li><?php } ?>
...
<article id="<?php echo $this->router->class; ?>-read"<?php if($this->input->cookie($this->router->class.'-read') == 'collapse') { ?> style="display:none;"<?php } ?>>
	<div class="column half">
		...
		<?php if($row->mbr_name) { ?><?php if($row->fle_owner == $this->phpcollab_member->mbr_id) { ?><i class="fa fa-<?php echo $this->config->item('phpcollab/icons/owner'); ?>" title="<?php echo $this->lang->line('icon_owner'); ?>"></i><?php } ?><?php echo $row->mbr_name; ?><?php } else { ?>-<?php } ?>
		</p>
		<p>
		...
		<?php if($row->fle_description) { ?><?php echo nl2br($row->fle_description); ?><?php } else { ?>-<?php } ?>
		</p>
	</div>
	<div class="column half">
		<p>
